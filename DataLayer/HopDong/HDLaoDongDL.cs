...
namespace DataLayer
{
    public class HDLaoDongDL : DataProvider
    {
        ...
        FROM HopDong hd 
        JOIN NhanVien nv ON hd.IdNhanVien = nv.IdNhanVien 
        WHERE DATEDIFF(DAY, GETDATE(), hd.NgayKetThuc) BETWEEN 0 AND @SoNgay";

            string connectionString = "Data Source=.;Initial Catalog=QuanLyNhanSu;Integrated Security=True";
            ...
            using (SqlConnection cn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, cn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@SoNgay", soNgayCanhBao);
        ...
        {
            string sql = @"INSERT INTO HopDong(IdHopDong, SoHD, NgayBatDau, NgayKetThuc, NgayKy, NoiDung, LanKy, ThoiHan, HeSoLuong,LuongCoBan, IdNhanVien, Created_By, Created_Date)
                           VALUES (@IdHopDong, @SoHD, @NgayBatDau, @NgayKetThuc, @NgayKy, @NoiDung, @LanKy, @ThoiHan, @HeSoLuong,@LuongCoBan, @IdNhanVien, @Created_By, @Created_Date)";
        ...
        public int Delete(string soHD)
        {
            string sql = "DELETE FROM HopDong WHERE SoHD = N'" + soHD.Replace("'", "''") + "'";
            try
            {
